#################################################### RF ####################################################
# Random Forests-MIDAS estimates for inflation nowcasts/forecasts
###############################################################################################################
...
for (n in 1:N){                                                     # Loop over out-of-sample periods
  ...
  for (i in 1:ndays){                                               # Loop over nowcasting days
    ...
    if (CVtreeFlag == 1){
      ...
      cv_model <- suppressWarnings(train(y = Y, x = as.matrix(X, ncol = ncol(X)), method = "rf",   # Train model using cross-validation specifications
                                         tuneGrid = tunegrid, trControl = myTimeControl, metric='RMSE'))
      
      best_mtry   <- as.numeric(cv_model$bestTune) # Obtain optimized mtry
      best_model  <- randomForest(y = Y, x= as.matrix(X, ncol = ncol(X)), mtry = best_mtry, ntree = 500) # Re-estimate best model
    } else {
      best_model  <- randomForest(y = Y, x= as.matrix(X, ncol = ncol(X)), ntree = 500) # Estimate best model with default mtry: max(floor(ncol(x)/3), 1)
    }
