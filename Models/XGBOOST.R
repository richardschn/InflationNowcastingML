#################################################### XGBOOST ####################################################
# XGBOOST-MIDAS estimates for inflation nowcasts/forecasts
###############################################################################################################
...
for (n in 1:N){                                                     # Loop over out-of-sample periods
  ...
  for (i in 1:ndays){                                               # Loop over nowcasting days
    ...
    # This part needs to change in case you want to use a different model
    ...
    if (CVtreeFlag == 1){
      ...
      tunegrid <- expand.grid(nrounds = c(1, 10), # Boosting iterations
                              max_depth = c(1, 4, 6), # Max Tree Depth
                              eta = c(.1, .4, .6), # Shrinkage
                              gamma = c(0,1,10), # Minimum loss reduction
                              colsample_bytree = c(.2,.7,1), # Subsample ratio of columns
                              min_child_weight = c(1,5), # Minimum sum of instance weight
                              subsample = c(.2, .8, 1)) # Subsample percentage
      cv_model <- suppressWarnings(train(y = Y, x = as.matrix(X, ncol = ncol(X)), method = "xgbTree",   # Train model using cross-validation specifications
                                         tuneGrid = tunegrid, trControl = myTimeControl, metric='RMSE', verbosity = 0))
      
      best_nrounds          <- as.numeric(cv_model$bestTune[1]) # Obtain optimized nrounds
      best_max_depth        <- as.numeric(cv_model$bestTune[2]) 
      best_eta              <- as.numeric(cv_model$bestTune[3]) 
      best_gamma            <- as.numeric(cv_model$bestTune[4]) 
      best_colsample_bytree <- as.numeric(cv_model$bestTune[5]) 
      best_min_child_weight <- as.numeric(cv_model$bestTune[6]) 
      best_subsample        <- as.numeric(cv_model$bestTune[7]) 
      
      xgboost_data <- xgb.DMatrix(data = as.matrix(X, ncol = ncol(X)), label = Y)
      
      best_model  <- xgboost(data = xgboost_data, 
                             ...
                             objective = "reg:squarederror") # Re-estimate best model
    } else {
      xgboost_data <- xgb.DMatrix(data = as.matrix(X, ncol = ncol(X)), label = Y)
      best_model   <- xgboost(data = xgboost_data, 
                              nrounds = 100,
                              early_stopping_rounds = 10,
                              objective = "reg:squarederror") # Estimate best model with default parameters
    }
    ...
    Yhat  <- predict(best_model, newdata = as.matrix(Xpred, ncol = ncol(Xpred))) # Nowcast/forecast for Y_t+h
  ...
  if (n %% 10 == 0){
    ...
    modelSpecs       <- paste0(modelName, "_focusFlag", focusFlag, "_rollwdwFlag", rollwdwFlag, "_ragedgeFlag",ragedgeFlag, "_onehqFlag", onehqFlag, "_weightFlag", weightFlag*weightWindow, "_CV", CVtreeFlag)
    setwd(paste0(path, "/Results/", modelName))
    save.image(file=paste0('env_', modelSpecs,'.RData'))
  }
