...
for (n in 1:N){                                                     # Loop over out-of-sample periods
  ...
  for (i in 1:ndays){                                               # Loop over nowcasting days
    ...
    if (CVtreeFlag == 1){    
    ll.forest  <- ll_regression_forest(Y=Y,
                                       X= as.matrix(X[, sel_coef[-1]], ncol = ncol(X[, sel_coef[-1]])),
                                       tune.parameters = "all")
    best_sample_fraction <- as.numeric(ll.forest$tunable.params[1])
    best_mtry            <- as.numeric(ll.forest$tunable.params[2])
    best_min_node_size   <- as.numeric(ll.forest$tunable.params[3])
    best_honesty_frac    <- as.numeric(ll.forest$tunable.params[4])
    best_honest_leaves   <- as.numeric(ll.forest$tunable.params[5])
    best_alpha           <- as.numeric(ll.forest$tunable.params[6])
    best_imbalance       <- as.numeric(ll.forest$tunable.params[7])
    
    # best model 
    best_model  <- ll_regression_forest(Y = Y,
                                        X= as.matrix(X[, sel_coef[-1]], ncol = ncol(X[, sel_coef[-1]])),
                                        sample.fraction = best_sample_fraction,
                                        mtry = best_mtry,
                                        min.node.size = best_min_node_size,
                                        honesty.fraction = best_honesty_frac,
                                        honesty.prune.leaves = best_honest_leaves,
                                        alpha = best_alpha,
                                        imbalance.penalty = best_imbalance)  
    } else {
      best_model  <- ll_regression_forest(Y = Y,
                                          X= as.matrix(X[, sel_coef[-1]], ncol = ncol(X[, sel_coef[-1]]))) 
    }
    ...
    Yhat  <- predict(best_model, newdata = as.matrix(Xpred[, sel_coef[-1]], ncol = ncol(Xpred[, sel_coef[-1]])))$prediction # Nowcast/forecast for Y_t+h
    ...
    if (MoMFlag==1){                                 
      Yhat   <- exp2YoY(unlist(c(y[(N0+n-12):(N0+n-2)],Yhat[1]), use.names = FALSE))       # Forecast of the YoY inflation rate via implied MoM
