#################################################### GRF ####################################################
# GRF-MIDAS estimates for inflation nowcasts/forecasts using the grf package
###############################################################################################################
...
for (n in 1:N){                                                     # Loop over out-of-sample periods
  ...
  for (i in 1:ndays){                                               # Loop over nowcasting days
    ...
    if (CVtreeFlag == 1){    
        grf.forest  <- grf::regression_forest(Y=Y,
                                              X= as.matrix(X, ncol = ncol(X)),
                                              tune.parameters = "all")
        best_sample_fraction <- as.numeric(grf.forest$tunable.params[1])
        best_mtry            <- as.numeric(grf.forest$tunable.params[2])
        best_min_node_size   <- as.numeric(grf.forest$tunable.params[3])
        best_honesty_frac    <- as.numeric(grf.forest$tunable.params[4])
        best_honest_leaves   <- as.numeric(grf.forest$tunable.params[5])
        best_alpha           <- as.numeric(grf.forest$tunable.params[6])
        best_imbalance       <- as.numeric(grf.forest$tunable.params[7])

        # best model 
        best_model  <- grf::regression_forest(Y = Y,
                                              X= as.matrix(X, ncol = ncol(X)),
    ...
    } else { # In case of no cross validation
      best_model  <- grf::regression_forest(Y = Y,
                                            X= as.matrix(X, ncol = ncol(X)))
    }
    ...
    Yhat  <- predict(best_model, newdata = as.matrix(Xpred, ncol = ncol(Xpred)))$prediction      # Nowcast/forecast for Y_t+h                                               
