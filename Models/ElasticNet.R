#################################################### ELASTIC NET ####################################################
# EN-MIDAS estimates for inflation nowcasts/forecasts
###############################################################################################################
...
coefnames   <- c("Const",paste0("Y",1:p),xRegr)                               # Name regressors
...
coefDF        <- data.frame(matrix(rep(NA, 3*N), ncol=3))                     # Only for glmnet: save model best coef in cv
names(coefDF) <- c("alpha", "lambda", "RMSEin")
coefDF[,1]    <- rep(1, N)
...
for (n in 1:N){                                                     # Loop over out-of-sample periods
  ...
  for (i in 1:ndays){                                               # Loop over nowcasting days
    ...
    grid_lambda <- 10 ^ seq(1, -5, length = 100)                                                # Specify grid for lambda
    grid_alpha  <- seq(0, 1, length = 5) 
    cv_model <- suppressWarnings(train(y = Y, x = as.matrix(X, ncol = ncol(X)), method = "glmnet", # Train model using cross-validation specifications
                                       tuneGrid = expand.grid(alpha = grid_alpha, lambda = grid_lambda), trControl = myTimeControl, metric='RMSE'))
    best_lambda <- as.numeric(cv_model$bestTune[2])                                             # Obtain optimized lambda
    best_alpha  <- as.numeric(cv_model$bestTune[1])                                             # Obtain optimized alpha
    best_model  <- glmnet(X, Y, alpha = best_alpha, lambda = best_lambda)                       # Re-estimate best model
    
    # Saves optimal parameters and RMSE in sample
    coefDF[n, 1] <- best_alpha
    coefDF[n, 2] <- best_lambda
    coefDF[n, 3] <- cv_model$results[ which(cv_model$results[,1] == best_alpha & cv_model$results[,2] == best_lambda) ,3]
    
    # Save coefficients
    coef_opt <- as.matrix(coef(best_model))                               # Obtain coefficients of optimized model
    rownames(coef_opt) <- c("Const",colnames(X))                          # Name coefficients
    coef_matrix[[i]][n,rownames(coef_opt)] <- coef_opt                    # Save coefficients for oos period n and relevant nowcasting day i
    ...
    Yhat  <- predict(best_model,                                        # Nowcast/forecast for Y_t+h
                     s = best_lambda, newx = data.matrix(Xpred))                                                
    
    # Focus average expectations
    Yfocus <- xexp[(N0+n-1),focusRegr[i]]                               # Nowcast of market expectations
...
if (saveFlag == 1) {
  ...
  fileName_cv      <- paste0("cv_", modelSpecs, ".xlsx")
  ...
  for (i in 1:length(coef_matrix)){
    xlsx::write.xlsx(data.frame(coef_matrix[[i]]), file=fileName_coef, sheetName=paste0("Day_",mdays[i]), 
                     row.names = FALSE, append=TRUE)
  }
  xlsx::write.xlsx(dummyTab, file=fileName_coef, sheetName="ModelSpec", row.names = FALSE, append=TRUE)
  
  xlsx::write.xlsx(data.frame(coefDF), file=fileName_cv, sheetName="coefDF", 
                   row.names = FALSE, append=FALSE)
  xlsx::write.xlsx(dummyTab, file=fileName_cv, sheetName="ModelSpec", 
                   row.names = FALSE, append=TRUE)
}