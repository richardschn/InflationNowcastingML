#################################################### BART2 ####################################################
# BART-MIDAS estimates for inflation nowcasts/forecasts using the rBART package
###############################################################################################################
...
for (n in 1:N){                                                     # Loop over out-of-sample periods
  ...
  for (i in 1:ndays){                                               # Loop over nowcasting days
    ...
    # The hBART package already saves the best model
    best_model <- rbart(x.train = X, y.train = Y, ntree=200, ntreeh=1, ndpost=1000,
                        nskip=100, k=2, power=2.0, base=.95, tc=1, sigmav=rep(1,length(Y)),
                        fmean=mean(Y), overallsd = sd(Y), overallnu=10,
                        chv = cor(X,method="spearman"), pbd=c(.7,0.0), pb=.5, stepwpert=.1, probchv=.1,
                        minnumbot=5, printevery=1000, numcut=100, xicuts=NULL, nadapt=1000, adaptevery=100,
                        summarystats=FALSE)
    ...
    Yhat  <- predict(best_model, x.test = Xpred)$mmean                  # Nowcast/forecast for Y_t+h                                                
