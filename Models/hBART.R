#################################################### hBART ####################################################
# heteroskedastic BART-MIDAS estimates for inflation nowcasts/forecasts using the rBART package
###############################################################################################################
...
for (n in 1:N){                                                     # Loop over out-of-sample periods
  ...
  for (i in 1:ndays){                                               # Loop over nowcasting days
    ...
    best_model <- rbart(x.train = X, y.train = Y, ntree=200, ntreeh=40, ndpost=1000,
                        nskip=100, k=2, power=2.0, base=.95, tc=1, sigmav=rep(1,length(Y)),
                        fmean=mean(Y), overallsd = sd(Y), overallnu=10,
                        chv = cor(X,method="spearman"), pbd=.7, pb=.5, stepwpert=.1, probchv=.1,
