...
#' @param gindex  group membership of each covariate.
#' @param gammas sg-LASSO mixing parameter. \ifelse{html}{\out{<code>gamma_w = 1</code>}}{\code{gamma_w = 1}} is LASSO and \ifelse{html}{\out{<code>gamma_w = 0</code>}}{\code{gamma_w = 0}} group LASSO.
...
midasml_forecast <- function(y_in, y_out, x_in, x_out, gindex, y_out_dates, scheme, verbose = FALSE, gammas = NULL, nfolds=NULL, ...){
  y_hat <- numeric(length(y_out))
  if (is.null(gammas)){
    gammas <- seq(1, 0, length = 21)
  }
    for (i in 1:length(y_out)) {
      ...
      if(i==1){
        ...
        if(scheme == "expand"){# expanding window scheme:
          x <- rbind(x_in,x_out[(1:i-1),])
          y <- c(y_in, y_out[(1:i-1)]) 
        } 
        if(scheme == "rolling"){
          x <- rbind(x_in[-c(1:(i-1)),],x_out[(1:i-1),]) 
          y <-  c(y_in[-c(1:(i-1)),], y_out[(1:i-1)])
        }
        if (scheme == "real-time"){
          x <- x_in
          y <- y_in
      ...
      if (length(gammas)>1){
        ...
        for (i_gamma in gammas) {
          tmp <- cv.sglfit(x = x, y = y, gamma = i_gamma, gindex = gindex, nfolds = nfolds)
          cvms <- c(cvms, min(tmp$cvm))
        }
        gamma <- gammas[which(min(cvms) == cvms)]
        fit <- cv.sglfit(x = x, y = y, gamma = gamma, gindex = gindex, nfolds = nfolds)
      } else {
        fit <- cv.sglfit(x = x, y = y, gamma = gammas, gindex = gindex, nfolds = nfolds)
      }
      y_hat[i] <- as.numeric(predict(fit, newx = as.matrix(x_out[i,])))
    }
  return(y_hat)
}
